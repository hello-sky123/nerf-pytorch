...
def create_nerf(args):
    ...
    if args.dataset_type != 'llff' or args.no_ndc:
        ...
        render_kwargs_train['lin_disp'] = args.lin_disp
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0., white_bkgd=False, pytest=False):
    ...
    dists = torch.cat([dists, torch.tensor([1e10], dtype=dists.dtype, device=dists.device).expand(
    ...
    if raw_noise_std > 0.:
        ...
        if pytest:
            ...
            noise = torch.tensor(noise, dtype=raw.dtype, device=raw.device)
...
def render_rays(ray_batch,
    ...
    if perturb > 0.:
        ...
        if pytest:
            ...
            t_rand = torch.tensor(t_rand, dtype=z_vals.dtype, device=z_vals.device)
...
#     raw = run_network(pts)
    ...
    if n_importance > 0:
        ret['rgb0'], ret['disp0'], ret['acc0'] = rgb_map, disp_map, acc_map
...
def config_parser():
    ...
    parser.add_argument("--net_depth_fine", type=int, default=8,
                        help='layers in fine network')
    parser.add_argument("--net_width_fine", type=int, default=256,
    ...
    parser.add_argument("--multi_res", type=int, default=10,
                        help='log2 of max freq for positional encoding (3D location)')
    parser.add_argument("--multi_res_views", type=int, default=4,
                        help='log2 of max freq for positional encoding (2D direction)')
    parser.add_argument("--raw_noise_std", type=float, default=0.,
                        help='std dev of noise added to regularize sigma_a output, '
                             '1e0 recommended')
    ...
    # training options
    parser.add_argument("--pre_crop_iters", type=int, default=0,
                        help='number of steps to train on central crops')
    parser.add_argument("--pre_crop_frac", type=float,
    ...
    parser.add_argument("--lin_disp", action='store_true',
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        hwf = poses[0, :3, -1]
        poses = poses[:, :3, :4]
        print('Loaded llff', images.shape, render_poses.shape, hwf, args.data_dir)
        if not isinstance(i_test, list):
            i_test = [i_test]

        if args.llff_hold > 0:
            print('Auto LLFF holdout,', args.llff_hold)
            i_test = np.arange(images.shape[0])[::args.llff_hold]
        ...
        # images[..., :3]，... 与维度数有关，:3 匹配一个维度，... 匹配剩余任意数量的维度
        # R = αR_f + (1−α)R_b
        if args.white_bkgd:
        ...
        print('Loaded deepvoxels', images.shape, render_poses.shape, hwf, args.data_dir)
        i_train, i_val, i_test = i_split
    ...
    for i in trange(start, n_iters):
        ...
        if i % args.video_interval == 0 and i > 0:
